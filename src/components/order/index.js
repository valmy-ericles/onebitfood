    ...
    if (this.props.restaurant == undefined || this.props.order.length <= 0) {
      return(
        <Column size={6} offset={6}>
          <Title size={6} className="has-text-custom-gray-darker">Carrinho Vazio</Title>
          <Title size={6} className="has-text-custom-gray-darker">Escolha um produto</Title>
          <br />
        </Column>
      )
    } else {
                ...
                return <Fragment>
                        <Column.Group multiline gapless>
                          ...
                          <Column size="three-fifths">
                            <Title size={7} className="has-text-custom-gray-darker">
                              { orderItem.product.description }
                            </Title>
                          </Column>
                          <Column size="two-fifths" className="has-text-right">
...
const mapStateToProps = store => ({
  restaurant: store.NewOrderReducer.restaurant,
  order: store.NewOrderReducer.order
});
